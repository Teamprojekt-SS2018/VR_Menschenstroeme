...
namespace Valve.VR.InteractionSystem
{
    ...
    public class Teleport : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (visible)
            {
                if (WasTeleportButtonReleased(player.hands[0]))
                {
                    if (pointerHand == player.hands[0]) //This is the pointer hand
                    {
                        TryTeleportPlayer();
                    }
                }
            ...
            if (WasTeleportButtonPressed(player.hands[0]))
            {
                newPointerHand = player.hands[0];
            }
        ...
        private IEnumerator TeleportHintCoroutine()
        {
            ...
            while (true)
            {
                ...
                {
                    if (hand == player.hands[0])
                    {
                        bool showHint = IsEligibleForTeleport(player.hands[0]);
                        bool isShowingHint = !string.IsNullOrEmpty(ControllerButtonHints.GetActiveHintText(player.hands[0], EVRButtonId.k_EButton_SteamVR_Touchpad));
                        if (showHint)
                        {
                            if (!isShowingHint)
                            {
                                ControllerButtonHints.ShowTextHint(player.hands[0], EVRButtonId.k_EButton_SteamVR_Touchpad, "Teleport");
                            ...
                            if (Time.time > prevHapticPulseTime + 0.05f)
                            {
                                ...
                                pulsed = true;
                                player.hands[0].controller.TriggerHapticPulse(500);
                            }
                        }
                    ...
                    {
                        bool showHint = true;
                        bool isShowingHint = !string.IsNullOrEmpty(ControllerButtonHints.GetActiveHintText(player.hands[1], EVRButtonId.k_EButton_SteamVR_Touchpad));
                        if (showHint)
                        {
                            if (!isShowingHint)
                            {
                                ControllerButtonHints.ShowTextHint(player.hands[1], EVRButtonId.k_EButton_SteamVR_Touchpad, "Play/Pause\nReverse      Forward\nReset");
                                prevBreakTime = Time.time;
                                prevHapticPulseTime = Time.time;
                            }
                            ...
                            if (Time.time > prevHapticPulseTime + 0.05f)
                            {
                                //Haptic pulse for a few seconds
                                pulsed = true;
                                player.hands[1].controller.TriggerHapticPulse(500);
                            }
                        }
                        else if (!showHint && isShowingHint)
                        {
                            ControllerButtonHints.HideTextHint(player.hands[1], EVRButtonId.k_EButton_SteamVR_Touchpad);
                        }
                    }
                }
